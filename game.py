...
class TicTacToe:
    ...
    def available_moves(self):
        ...
        for (i, spot) in enumerate(self.board):   # creates a list and assign tuples that have a comma at the value of that index
            # ['x', 'x', 'o'] -->[(0, 'x'), 1, 'x'), (2, 'o')] #the for loop will go through each of these tuples
            if spot == ' ':   # available moves to be played
        ...
        while game.empty_squares():
            ...
            if game.make_move(square, letter):
                if print_game:
                    ...
                    game.print_board()  # how the board looks with the spaces have been occupied
                    print('')   # just empty line

                if game.current_winner:
                    if print_game:
                        print('YOU' + letter + 'RULE!')
                    return letter
